    ...
    return (
        <div className={styles.calendar}>
            <div className={styles.month}>
                {position === 'left' && <i onClick={onArrow} className="fas fa-chevron-left"></i>}
                <span >{date.locale('th').add(543, 'year').format('MMMM YYYY')}</span>
                {position === 'right' && <i onClick={onArrow} className="fas fa-chevron-right"></i>}
            </div>
...
const Date = () => {
    ...
    const [currentRageDate, setCurrentRangeDate] = useState<dayjs.Dayjs[]>([])
    ...
    useEffect(() => {
        // if()
        if (currentRageDate.every(item => item)) setRageDate(currentRageDate)
    }, [currentRageDate])
    return (
